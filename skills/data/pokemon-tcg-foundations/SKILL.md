...
description: Use when working with a trading-card sales tape — any sale-level dataset of collectible card transactions across multiple venues (Pokémon TCG, sports cards, MTG, or similar) — for price estimation, momentum and value factor construction, set/era-level regime detection, cross-venue basis and anchor-follower models, buy/sell signal design, backtesting card selection, or ML/quant research on card markets as an asset class. Encodes the structural defects these tapes reliably have (competing condition encodings, venue coverage gaps, lot-price contamination, FX-converted venues, venues that aren't secondary markets, silent writer regressions), the era structure and panel sparsity that makes naive panels unusable, the cross-grade price reconstruction that fixes it, the traps that produce wrong-but-plausible results, and the asset-class properties that break standard modeling assumptions (endogenous observation, cold start as the default, effective-breadth collapse, no shorting, non-back-fillable history). Also covers who is asking — collector, flipper, vendor, grader — the hobby's vocabulary mapped to columns, and a register of folk claims marked confirmed, retracted or untested. Includes measured coefficients from a reference implementation. Don't use for TCG game strategy/deckbuilding, general lakehouse engineering (that's data-apache-lakehouse), generic DuckDB syntax (data-duckdb), or non-card asset classes.
---
